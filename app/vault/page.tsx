...
import { VaultDTO, VaultStatusTS } from "@/lib/ironclad-service";
import TransitionButton from "@/components/navigation/TransitionButton";

function VaultCard({ vault }: { vault: VaultDTO }) {
  ...
  return (
    <div className="card-brutal p-6 hover:shadow-lg transition-shadow">
      ...
      <div className="space-y-3 body-brutal text-sm">
        ...
        {vault.expectedDeposit && (
          <div className="flex justify-between">
            <span className="text-gray-600">Expected Deposit:</span>
            <span className="font-bold">{formatSats(vault.expectedDeposit)}</span>
          </div>
        )}

        <div className="flex justify-between">
          <span className="text-gray-600">Lock Until:</span>
          <span className="font-bold">{formatDate(vault.lockUntil)}</span>
        </div>

        <div className="flex justify-between">
          <span className="text-gray-600">BTC Address:</span>
          <span className="font-mono text-xs">{vault.btcAddress.slice(0, 12)}...</span>
        </div>
      </div>

      {/* Action Buttons */}
      <div className="mt-4 flex gap-2">
        {vault.status === "Unlockable" && (
          <TransitionButton
            href="/vault/withdraw"
...
export default function VaultPage() {
  ...
  const activeVaults = vaults.filter((v) => v.status === "ActiveLocked");
  const unlockableVaults = vaults.filter((v) => v.status === "Unlockable");
  const pendingVaults = vaults.filter((v) => v.status === "PendingDeposit");
              ...
              {!isConnected ? (
                ...
                <>
                  ...
                  <div className="card-brutal mb-6">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h1 className="heading-brutal text-3xl mb-2">YOUR VAULTS</h1>
                  ...
                  {error && (
                    <div className="card-brutal p-6 bg-red-50 border-red-300">
                      <h3 className="heading-brutal text-lg text-red-900 mb-2">ERROR</h3>
                    ...
                    <>
                      ...
                      <div className="card-brutal p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 body-brutal">
                          ...
                          <div>
                            <p className="text-gray-600 text-sm">Active Locked</p>
